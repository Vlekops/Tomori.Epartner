...
{
    public partial class IntegrationConfigComponent : ComponentBase
    {
        #region Override
        ...
        private string _CIVD = string.Empty;
        private string _SAP = string.Empty;
        #endregion
        ...
        private async Task GetData()
        {
            ...
            try
            {
                var res = await _Service.GetIntegration(Token.BaseApiUrl, Token.RawToken);
                if (res.Succeeded)
                {
                    _CIVD = res.Data.Civd;
                    _SAP = res.Data.Sap;                }
        ...
        private async Task Save()
        {
            ...
            try
            {
                var param = new IntegrationConfig
                {
                    Sap = _SAP,
                    Civd = _CIVD
                };

                StatusResponse res = await _Service.SaveIntegration(param, Token.BaseApiUrl, Token.RawToken);
