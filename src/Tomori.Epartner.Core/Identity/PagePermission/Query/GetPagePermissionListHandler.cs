...
{
    public class GetPagePermissionListRequest : ListRequest,IListRequest<GetPagePermissionListRequest>,IRequest<ListResponse<PagePermissionResponse>>
    {
    }
    internal class GetPagePermissionListHandler : IRequestHandler<GetPagePermissionListRequest, ListResponse<PagePermissionResponse>>
    {
        private readonly ILogger _logger;
        private readonly IMapper _mapper;
        private readonly IUnitOfWork<ApplicationDBContext> _context;
        public GetPagePermissionListHandler(
            ILogger<GetPagePermissionListHandler> logger,
            IMapper mapper,
        ...
        public async Task<ListResponse<PagePermissionResponse>> Handle(GetPagePermissionListRequest request, CancellationToken cancellationToken)
        {
            ListResponse<PagePermissionResponse> result = new ListResponse<PagePermissionResponse>();
            try
            {
				var query = _context.Entity<Tomori.Epartner.Data.Model.PagePermission>().AsQueryable();

				#region Filter
				Expression<Func<Tomori.Epartner.Data.Model.PagePermission, object>> column_sort = null;
				List<Expression<Func<Tomori.Epartner.Data.Model.PagePermission, bool>>> where = new List<Expression<Func<Tomori.Epartner.Data.Model.PagePermission, bool>>>();
        ...
        #region List Utility
		private (Expression<Func<Tomori.Epartner.Data.Model.PagePermission, bool>> where, Expression<Func<Tomori.Epartner.Data.Model.PagePermission, object>> order) ListExpression(string search, string field, bool is_where)
		{
			Expression<Func<Tomori.Epartner.Data.Model.PagePermission, object>> result_order = null;
			Expression<Func<Tomori.Epartner.Data.Model.PagePermission, bool>> result_where = null;
            if (!string.IsNullOrWhiteSpace(search) && !string.IsNullOrWhiteSpace(field))
            {
                ...
                switch (field)
                {
					...
					case "idpage" : 
						if(is_where){
							if (Guid.TryParse(search, out var _IdPage))
								result_where = (d=>d.IdPage == _IdPage);
								else
								result_where = (d=>d.IdPage == Guid.Empty);
						}
						else
							result_order = (d => d.IdPage);
					break;
					case "name" : 
						if(is_where){
