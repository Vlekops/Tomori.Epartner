...
namespace Tomori.Epartner.Core.Repository.Query
{
    ...
    internal class GetRepositoryListHandler : IRequestHandler<GetRepositoryListRequest, ListResponse<RepositoryResponse>>
    {
        ...
        public async Task<ListResponse<RepositoryResponse>> Handle(GetRepositoryListRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
				var query = _context.Entity<Tomori.Epartner.Data.Model.Repository>().AsQueryable();

				#region Filter
				Expression<Func<Tomori.Epartner.Data.Model.Repository, object>> column_sort = null;
				List<Expression<Func<Tomori.Epartner.Data.Model.Repository, bool>>> where = new List<Expression<Func<Tomori.Epartner.Data.Model.Repository, bool>>>();
        ...
        #region List Utility
		private (Expression<Func<Tomori.Epartner.Data.Model.Repository, bool>> where, Expression<Func<Tomori.Epartner.Data.Model.Repository, object>> order) ListExpression(string search, string field, bool is_where)
		{
			Expression<Func<Tomori.Epartner.Data.Model.Repository, object>> result_order = null;
			Expression<Func<Tomori.Epartner.Data.Model.Repository, bool>> result_where = null;
            if (!string.IsNullOrWhiteSpace(search) && !string.IsNullOrWhiteSpace(field))
            {
                ...
                switch (field)
                {
                    ...
                    case "moduls":
                        if (is_where)
                        {
							var moduls = JsonConvert.DeserializeObject<List<string>>(search);
                            result_where = (d => moduls.Contains(d.Modul.Trim().ToLower()));
                        }
                        else
                            result_order = (d => d.Modul);
                        break;
