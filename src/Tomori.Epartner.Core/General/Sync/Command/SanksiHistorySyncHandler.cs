...
namespace Tomori.Epartner.Core.General.Sync.Command
{
    ...
    internal class SanksiHistorySyncHandler : IRequestHandler<SanksiHistorySyncRequest, StatusResponse>
    {
        ...
        public SanksiHistorySyncHandler(ILogger<SanksiHistorySyncHandler> logger, IMapper mapper, IMediator mediator, IUnitOfWork<ApplicationDBContext> context, ICIVDAPIHelper restHelper)
        {
            _logger = logger;
            _mapper = mapper;
            _mediator = mediator;
        ...
        public async Task<StatusResponse> Handle(SanksiHistorySyncRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var listInsert = new List<Data.Model.VendorSanksi>();
                ...
                if (rest.success)
                {
                    ...
                    {
                        var insert = new Data.Model.VendorSanksi();
                        Guid? IdVendor = null;
                        ...
                        var s = await _context.Entity<Data.Model.VendorSanksi>().Where(a => a.CivdId == data.id).FirstOrDefaultAsync();
                        if (s == null)
                        {
