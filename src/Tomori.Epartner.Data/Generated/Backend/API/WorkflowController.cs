...
namespace Tomori.Epartner.API.Controllers
{
    public partial class WorkflowController : BaseController<WorkflowController>
    {
        ...
        {
            return Wrapper(await _mediator.Send(new GetWorkflowByIdRequest() { Id = id }), new { id });
        }
        ...
        public async Task<IActionResult> Add([FromBody] WorkflowRequest request)
        {
            var add_request = _mapper.Map<AddWorkflowRequest>(request);
            add_request.Token = Token.User;
            return Wrapper(await _mediator.Send(add_request), request);
        }

        [HttpPut(template: "edit/{id}")]
        public async Task<IActionResult> Edit(Guid id, [FromBody] WorkflowRequest request)
        {
            var edit_request = _mapper.Map<EditWorkflowRequest>(request);
            edit_request.Id = id;
        ...
        {
            return Wrapper(await _mediator.Send(new DeleteWorkflowRequest() { Id = id, Token = Token.User }), new { id });
        }
