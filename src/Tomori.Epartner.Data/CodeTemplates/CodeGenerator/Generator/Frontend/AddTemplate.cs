...
namespace Tomori.Epartner.Data.CodeGenerator.Generator
{
    ...
    {
        #region Template
        public static (bool success, string key, object val) GenerateViewAdd(string current_namespace, IModel model, Microsoft.Extensions.Options.IOptions<HandlebarsScaffoldingOptions> _options, List<SettingExcludeTableCodeGeneratorObject> exclude)
        {
            ...
            string template_path = Path.Combine(project_path, current_namespace + @"Data\CodeTemplates\CodeGenerator\Template\Frontend\ViewAdd.hbs");
            ...
            using (sb.Indent())
            {
                foreach (var entityType in model.GetScaffoldEntityTypes(_options.Value))
                {
                    ...
                    if (create_code)
                    {
                        ...
                        string code_file = Path.Combine(target_path, $"_Add.cshtml");
                        ...
                        foreach (var d in list_properties)
                        {
                            ...
                            if (!exclude_attributes.Contains(d.Name.ToLower()))
                            {
                                ...
                                {
                                    case "DateTime":
                                        attribute += tabular + "\t" + @$"<input id=""Add-{name}-{d.Name}"" type=""text"" class=""form-control datepicker clear"" placeholder=""dd-MM-yyyy""/>";
                                        break;
                                    case "bool":
                                        attribute += tabular + "\t" + @$"<input id=""Add-{name}-{d.Name}"" type=""checkbox"" class=""form-control clear""/>";
                                        break;
                                    default:
                                        attribute += tabular + "\t" + @$"<input id=""Add-{name}-{d.Name}"" class=""form-control form-control-sm clear""/>";
                                        break;
                                }
