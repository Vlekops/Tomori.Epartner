...
namespace Tomori.Epartner.Data.CodeGenerator.Generator
{
    ...
    {
        #region Template
        public static (bool success, string key, object val) GenerateScriptAdd(string current_namespace, IModel model, Microsoft.Extensions.Options.IOptions<HandlebarsScaffoldingOptions> _options, List<SettingExcludeTableCodeGeneratorObject> exclude)
        {
            ...
            string template_path = Path.Combine(project_path, current_namespace + @"Data\CodeTemplates\CodeGenerator\Template\Frontend\ScriptsAdd.hbs");
            ...
            using (sb.Indent())
            {
                foreach (var entityType in model.GetScaffoldEntityTypes(_options.Value))
                {
                    ...
                    if (create_code)
                    {
                        ...
                        string code_file = Path.Combine(target_path, $"add.js");
                        ...
                        foreach (var d in list_properties)
                        {
                            ...
                            if (!exclude_attributes.Contains(d.Name.ToLower()))
                            {
                                ...
                                {
                                    case "bool":
                                        attribute = tabular + @$"{att_name}:$('#Add-{name}-{d.Name}').is("":checked""),";
                                        break;
                                    default:
                                        attribute = tabular + @$"{att_name}:$('#Add-{name}-{d.Name}').val(),";
                                        break;
                                }
