...
namespace Tomori.Epartner.Data.CodeGenerator.Generator
{
    ...
    {
        public static (bool success, string key, object val) GenerateInfrastructureResponse(string current_namespace, IModel model, Microsoft.Extensions.Options.IOptions<HandlebarsScaffoldingOptions> _options, List<SettingExcludeTableCodeGeneratorObject> exclude)
        {
            ...
            string template_path = Path.Combine(project_path, current_namespace + @"Data\CodeTemplates\CodeGenerator\Template\Infrastructure\Response.hbs");
            ...
            using (sb.Indent())
            {
                foreach (var entityType in model.GetScaffoldEntityTypes(_options.Value))
                {
                    ...
                    if (create_code)
                    {
                        ...
                        string target_path = Path.Combine(project_path, current_namespace + $@"Data\Generated\Infrastructure\Response\{GetPrefix(entityType.Name)}");
