...
namespace Tomori.Epartner.Data.CodeGenerator.Generator
{
    ...
    {
        #region Template
        public static (bool success, string key, object val) GenerateInfrastructureInterface(string current_namespace, IModel model, Microsoft.Extensions.Options.IOptions<HandlebarsScaffoldingOptions> _options, List<SettingExcludeTableCodeGeneratorObject> exclude)
        {
            ...
            string template_path = Path.Combine(project_path, current_namespace + @"Data\CodeTemplates\CodeGenerator\Template\Infrastructure\Interface.hbs");
            ...
            using (sb.Indent())
            {
                foreach (var entityType in model.GetScaffoldEntityTypes(_options.Value))
                {
                    ...
                    if (create_code)
                    {
                        ...
                        string target_path = Path.Combine(project_path, current_namespace + $@"Data\Generated\Infrastructure\Interface\{GetPrefix(entityType.Name)}");

                        if (!Directory.Exists(target_path))
                        {
                            Directory.CreateDirectory(target_path);
                        }
                        string service_file = Path.Combine(target_path, $"I{name}Service.cs");
